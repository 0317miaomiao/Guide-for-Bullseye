...
#SBATCH --output=dedup_mut_%j.out
#SBATCH --error=dedup_mut_%j.err

source activate bullseye_env

INPUT_DIR="bam_mutoutput"
OUTPUT_DIR="bam_mutsort"
...
find "$INPUT_DIR" -maxdepth 1 -type d -name 'SRR*' | xargs -n 1 basename > srr_list_mut_dedup.txt

# 并行执行，每次最多 4 个任务
cat srr_list_mut_dedup.txt | xargs -P 8 -n 1 -I {} bash -c 'process_sample "$@"' _ {}
