#!/bin/bash
#SBATCH --job-name=flexbar_batch
#SBATCH -p kshcnormal
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --time=7-00:00:00
#SBATCH --output=batch_flexbar_mut_%j.out
#SBATCH --error=batch_flexbar_mut_%j.err

source activate bullseye_pre

# 设置主目录
FASTQ_DIR="APOBEC1-YTHmut_fastq"
OUTPUT_DIR="trimmed_mut"
...
# 找所有SRR目录名称，写入临时文件
find "$FASTQ_DIR" -maxdepth 1 -type d -name 'SRR*' | xargs -n 1 basename > srr_list_mut.txt

cat srr_list_mut.txt | xargs -P 4 -n 1 -I {} bash -c 'download_srr "$@"' _ {}